    ...
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      ...
      <section className="px-4 py-6">
        <div className="container mx-auto">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              {activeCategories.slice(0, 6).map(cat => <CategoryCard key={cat.id} category={cat} />)}
            </div>
          )}
        </div>
      </section>
      ...
      <footer className="bg-card border-t border-border py-6 px-4">
        <div className="container mx-auto text-center space-y-2">
          <p className="font-heading text-lg font-bold gold-text">{settings?.org_name || "BS Marble Karachi"}</p>
          <p className="text-xs text-muted-foreground">Buy Premium Quality Marble at Affordable Prices</p>
          <p className="text-[11px] text-muted-foreground">Powered by Supabase — live data</p>
        </div>
      </footer>
